...
void MemberDelete() {
	...
	if (find_flag==-1) {
		...
		for (int i = 0; i < delpurchasecount; i++) {
			...
			delSizePur += PurchaseFile.Delete(PMwithMember[input][i]);
			num_purchase--;
		}
...
void MemberDelete(string user_id) {
	...
	if (find_flag==-1) {
		...
		for (int i = 0; i < delpurchasecount; i++) {
			...
			delSizePur += PurchaseFile.Delete(PMwithMember[user_id][i]);
			num_purchase--;
		}
...
void StockDelete() {
	...
	if (find_flag == 0) {
		...
		for (int i = 0; i < delpurchasecount; i++) {
			delSizePur += PurchaseFile.Delete(PMwithStock[input][i]);
			num_purchase--;
		}
...
void PurchaseDelete() {
	...
	if (idx == 0) {
		...
		for(int i=0;i<PMwithMember[p.getMemberId()].size();i++){
			if(PMwithMember[p.getMemberId()][i]==idx) {
